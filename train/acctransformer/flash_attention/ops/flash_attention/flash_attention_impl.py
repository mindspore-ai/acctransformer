...
def get_flash_attention_grad(prev_block_num=65536, next_block_num=65536, tiling_stgy_name='sparse'):
    """get flash attention grad"""

    def infer_shape(q_shape, k_shape, v_shape, o_shape, do_shape, l_shape,
                    att_mask_shape, dropout_mask_shape, alibi_mask_shape):
        return q_shape, k_shape, v_shape

    def infer_dtype(q_dtype, k_dtype, v_dtype, o_dytpe, do_dtype, l_dtype,
    ...
    fa_grad.init_prim_io_names(
        inputs=["query", "key", "value", "output", "do", "rowsum", "attn_mask", "dropout_mask", "alibi_mask"],
        outputs=["dq", "dk", "dv"]
    )

    def bprop(query, key, value, attn_mask, dropout_mask, alibi_mask, outs, douts):
        output, rowsum, = outs
        dout, _, = douts
        dq, dk, dv = fa_grad(query, key, value, output, dout, rowsum, attn_mask, dropout_mask, alibi_mask)
        dq = ops.cast(dq, mstype.float16)
        dk = ops.cast(dk, mstype.float16)
        dv = ops.cast(dv, mstype.float16)
        return dq, dk, dv, zeros_like(attn_mask), zeros_like(dropout_mask), zeros_like(alibi_mask)

    return bprop


def get_flash_attention(prev_block_num=65536, next_block_num=65536, tiling_stgy_name='sparse'):
    """get_flash_attention"""

    def infer_shape(q_shape, k_shape, v_shape, attn_mask_shape=None, dropout_mask_shape=None, alibi_mask_shape=None):
        """infer shape"""
        batch, head_num, seq_len, _ = q_shape
        l_shape = (batch, head_num, seq_len)
        return q_shape, l_shape

    def infer_dtype(q_dtype, k_dtype, v_dtype, attn_mask_dtype=None, dropout_mask_dtype=None, alibi_mask_type=None):
        """infer type"""
        return q_dtype, mstype.float32

    fa_grad = get_flash_attention_grad(prev_block_num, next_block_num, tiling_stgy_name)
