...
from accspeed.ops.flash_attention.constants import HARDWARE_MAX_STRIDE_LMT
from accspeed.ops.flash_attention.constants import MASK_FILL_VALUE
from accspeed.ops.flash_attention.constants import UB
from accspeed.ops.flash_attention.tik_ops_utils import TikOpsUtils
from accspeed.ops.flash_attention.tiling_strategy.sparse_tiling import SparseTiling
from accspeed.ops.flash_attention.tiling_strategy.strategy import TilingPara
...
class FlashAttention(metaclass=ABCMeta):
    ...
    def do_att_mask(self, Sij_ub_N1MN0, q_blk_height, kv_blk_height,
        ...
        with self.tik_instance.new_stmt_scope(disable_sync=False):
            ...
            self.tik_instance.data_move(att_mask_ub, self.att_mask_gm, 0,
                                        1, kv_blk_height * q_blk_height // 16,
                                        0, 0)
    ...
    def unroll_data_move(self, dst, dst_offset, src, src_offset, nburst, burst, src_stride, dst_stride,
        ...
        if dst_stride <= HARDWARE_MAX_STRIDE_LMT and src_stride <= HARDWARE_MAX_STRIDE_LMT:
            ...
            for _ in range(nburst):
                self.tik_instance.data_move(dst=dst[dst_offset],
                                            src=src[src_offset],
                                            sid=0,
                                            nburst=1,
                                            burst=burst,
